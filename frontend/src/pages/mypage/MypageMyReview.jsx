...
export default function MypageMyReview({myReview,setMyReview,getMyReview,formatKoreanDateTime}) {
  ...
  // 🆕 리뷰 개수가 바뀔 때는 항상 1페이지로 리셋
  useEffect(() => {
    setCurrentPage(1);
  }, [myReview.length]);

  // 🆕 전체 페이지 수
  const totalPages = Math.max(1, Math.ceil(myReview.length / PAGE_SIZE));

  // 🆕 현재 페이지가 전체 페이지를 넘어가지 않도록 보정
  const safePage = Math.min(currentPage, totalPages);

  // 🆕 현재 페이지에 보여줄 리뷰만 잘라내기
  const startIndex = (safePage - 1) * PAGE_SIZE;
  const endIndex = startIndex + PAGE_SIZE;
  const pagedMyReview = myReview.slice(startIndex, endIndex);
